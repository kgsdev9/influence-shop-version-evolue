            ...
            return {
                ...
                filterProducts() {
                    ...
                    this.filteredProducts = this.products.filter(product => {
                        ...
                        const categoryMatch = this.selectedCategories.length === 0 || this.selectedCategories.map(
                            String).includes(String(product.category_id));

                        // Filtrer par taille
                        const sizeMatch = this.selectedSizes.length === 0 || product.sizes.some(size => this
                            .selectedSizes.map(String).includes(String(size.id)));

                        // Filtrer par couleur
                        const colorMatch = this.selectedColors.length === 0 || product.colors.some(color => this
                            .selectedColors.map(String).includes(String(color.id)));

                        // Filtrer par prix
                        const priceMatch = product.price_vente >= this.priceRange[0] && product.price_vente <= this
                            .priceRange[1];

                        return categoryMatch && sizeMatch && colorMatch &&
                        priceMatch; // Le produit doit correspondre à tous les filtres
                    });
